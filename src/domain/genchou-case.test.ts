...
import { checkCustomerInfo, startCase, withCustomerInfo } from './genchou-case'

describe('案件を始めたときの顧客情報', () => {
  it('調査日に今日の日付が入っている', () => {
    const started = startCase({ today: '2026-09-16' })

    expect(started.customer.surveyedOn).toBe('2026-09-16')
  })

  it('担当者名に前回の担当者が入っている', () => {
    const started = startCase({ today: '2026-09-16', lastSurveyorName: '田中' })
...
describe('顧客情報の画面から次へ進めるか', () => {
  ...
  it('顧客名が空のままでは進めない', () => {
    const check = checkCustomerInfo(started.customer)
  ...
  it('顧客名が空白だけでも進めない', () => {
    ...
    expect(checkCustomerInfo(filled.customer).canProceed).toBe(false)
  })

  it('顧客名を入れれば進める', () => {
    const filled = withCustomerInfo(started, { customerName: '山田' })

    expect(checkCustomerInfo(filled.customer).canProceed).toBe(true)
  })
